...
let dic_promise = new Promise(
	function( dic_resolve, dic_reject) {
		(async function() {
			...
			try {
				let dictionary = {};
				let css = await fs.readFile( 'libs/themify-icons/themify-icons.css', 'utf8' );
				let match = null;
				...
				await fs.writeFile('output/themify/dictionary.min.json', JSON.stringify(dictionary));
				await fs.writeFile('output/themify/dictionary.json', JSON.stringify(dictionary, null, 2));
...
dic_promise.then(
	function() {
		// styles
		create_style_files({
			fonts: ['themify.eot', 'themify.ttf', 'themify.woff', 'themify.svg'],
			font_family: 'themify',
			class_prefix: 'themify-',
			json_file: 'output/themify/dictionary.min.json',
			output_directory: '../assets/themify/',
		});

		// fetch
		create_fetch_files({
			dictionary_file: 'output/themify/dictionary.min.json',
			output_directory: '../assets/themify/',
		});
		...
		create_library_files({
			dictionary_file: 'output/themify/dictionary.min.json',
			icons_directory: 'libs/themify-icons/SVG/',
			output_directory: '../assets/themify/',
