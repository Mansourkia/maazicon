...
let dic_promise = new Promise(
	function( dic_resolve, dic_reject) {
		(async function() {
			...
			try {
				let dictionary = {};
				let css = await fs.readFile( 'libs/Linearicons-Free-v1.0.0/Web Font/style.css', 'utf8' );
				let match = null;
				...
				await fs.writeFile('output/linearicons/dictionary.min.json', JSON.stringify(dictionary));
				await fs.writeFile('output/linearicons/dictionary.json', JSON.stringify(dictionary, null, 2));
...
dic_promise.then(
	function() {
		// styles
		create_style_files({
			fonts: ['Linearicons-Free.eot', 'Linearicons-Free.woff2', 'Linearicons-Free.woff', 'Linearicons-Free.ttf', 'Linearicons-Free.svg'],
			font_family: 'Linearicons',
			class_prefix: 'linearicons-',
			json_file: 'output/linearicons/dictionary.min.json',
			output_directory: '../assets/linearicons/',
		});

		// fetch
		create_fetch_files({
			dictionary_file: 'output/linearicons/dictionary.min.json',
			output_directory: '../assets/linearicons/',
		});
		...
		create_library_files({
			dictionary_file: 'output/linearicons/dictionary.min.json',
			icons_directory: 'libs/Linearicons-Free-v1.0.0/SVG/',
			output_directory: '../assets/linearicons/',
