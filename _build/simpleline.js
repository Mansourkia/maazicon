...
let dic_promise = new Promise(
	function( dic_resolve, dic_reject) {
		(async function() {
			...
			try {
				let dictionary = {};
				let css = await fs.readFile( 'libs/simple-line-icons-2.5.4/css/simple-line-icons.css', 'utf8' );
				let match = null;
				...
				await fs.writeFile('output/simpleline/dictionary.min.json', JSON.stringify(dictionary));
				await fs.writeFile('output/simpleline/dictionary.json', JSON.stringify(dictionary, null, 2));
...
dic_promise.then(
	function() {
		// styles
		create_style_files({
			fonts: ['Simple-Line-Icons.eot', 'Simple-Line-Icons.woff2', 'Simple-Line-Icons.woff', 'Simple-Line-Icons.ttf', 'Simple-Line-Icons.svg'],
			font_family: 'Simpleline',
			class_prefix: 'simpleline-',
			json_file: 'output/simpleline/dictionary.min.json',
			output_directory: '../assets/simpleline/',
		});

		// fetch
		create_fetch_files({
			dictionary_file: 'output/simpleline/dictionary.min.json',
			output_directory: '../assets/simpleline/',
		});
		...
		create_library_files({
			dictionary_file: 'output/simpleline/dictionary.min.json',
			icons_directory: 'libs/simple-line-icons-2.5.4/src/svgs/',
			output_directory: '../assets/simpleline/',
